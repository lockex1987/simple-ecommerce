 ...
 */
CV.tranlateErrorMessage = (msg: string) => {
  return msg
}
 ...
 */
CV.normalizePhoneNumber = (value: string) => {
 ...
 * @param {String} value Xâu đầu vào, là một ngày đã đúng định dạng
 */
CV.normalizeDate = (value: string) => {
 ...
 * Các định dạng ngày đúng định dạng là:
 ...
 * @return true nếu địa chỉ email đúng định dạng
 */
...
/**
 * Validate định dạng số điện thoại, chỉ bao gồm chữ số, có thể có dấu cách, dấu trừ, dấu cộng, dấu mở ngoặc, dấu đóng ngoặc.
 ...
 * Số điện thoại đã được chuẩn hóa thành 10 số.
 * @param value Giá trị text
 */
...
/**
 * Validate URL đúng định dạng.
 * @param value Giá trị URL
 * @returns true nếu giá trị là một URL đúng định dạng
 */
...
CV.parseValidationFromString = validationRules => {
  ...
  validationRules.split('|').forEach(rule => {
    ...
    if (s == 'required') {
      ...
      rules.email = true
      rules.emailMessage = (a.length > 1) ? a[1] : 'Email không đúng định dạng'
    } else if (s == 'phone') {
      rules.phone = true
      rules.phoneLengthMessage = 'Số điện thoại phải dài 10 chữ số, bạn nhập vào {0} chữ số'
      rules.phoneFormatMessage = 'Số điện thoại chỉ được bao gồm chữ số và khoảng trắng'
    } else if (s == 'integer') {
      rules.integer = true
      rules.integerMessage = (a.length > 1) ? a[1] : 'Số nguyên không đúng định dạng'
    } else if (s == 'numeric') {
      rules.numeric = true
      rules.numericMessage = (a.length > 1) ? a[1] : 'Số không đúng định dạng'
    } else if (s == 'date') {
      rules.date = true
      rules.dateMessage = (a.length > 1) ? a[1] : 'Ngày không đúng định dạng'
    } else if (s == 'idNumber') {
      rules.idNumber = true
      ...
      rules.domainNameMessage = (a.length > 1) ? a[1] : 'Tên miền không đúng định dạng'
    } else if (s == 'url') {
      rules.url = true
      rules.urlMessage = (a.length > 1) ? a[1] : 'URL không đúng định dạng'
    } else if (s == 'password') {
      rules.password = true
      ...
      rules.fileTypes = a[1]
      rules.fileTypesMessage = (a.length > 2) ? a[2] : 'Vui lòng chọn file có định dạng: {0}'
    } else if (s == 'in') {
      console.log('in')
      rules.valueIn = a[1]
...
CV.checkType = (value, validation) => {
  ...
  if (validation.phone) {
    ...
    const normalizedPhone = CV.normalizePhoneNumber(value)
    if (!CV.validatePhoneLength(normalizedPhone)) {
      return validation.phoneLengthMessage.format(normalizedPhone.length)
    }
  }
 ...
 * @param {DOMNode} form Một phần tử DOM
 */
...
CV.clearSingleErrorMessage = el => {
  ...
  container.querySelectorAll('.error-message').forEach(err => {
    err.remove()
  })
}
 ...
 * @param {DOMNode} el Đối tượng DOM (input, textarea, select)
...
CV.showError = (el, errorMessage) => {
  ...
  message.textContent = Array.isArray(errorMessage)
    ? errorMessage.join('\n')
    : errorMessage
...
CV.getValidateError = el => {
  ...
  if (el.dataset.ignoreValidate == 'true') {
    return null
  }

  if (!el.dataset.alwaysValidate
        || el.dataset.alwaysValidate == 'false') {
  ...
  const errorMessage = CV.validateByValueAndValidation(el.value, el.validation)
        || CV.checkFileSizeAndType(el)
        || ''
  return errorMessage
}
...
CV.validateByValueAndValidation = (value, validation) => {
  ...
  let errorMessage = CV.checkRequired(value, validation)
        || CV.checkGroupRequired(validation)
        || ''

  if (!errorMessage) {
    errorMessage = CV.checkLength(value, validation)
            || CV.checkType(value, validation)
            || CV.checkRange(value, validation)
            || CV.checkPattern(value, validation)
            || CV.checkMatch(value, validation)
 ...
 * @param {DOMNode} form Phần tử DOM
 * @return {Boolean} true nếu form KHÔNG hợp lệ
 */
CV.invalidForm = form => {
  ...
  const elements = form.querySelectorAll('.validate-container input, '
 ...
 * TODO: Debounce, không thông báo lỗi luôn khi người dùng đang nhập.
 */
